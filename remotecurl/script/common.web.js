...
const create_proxied_web_object = function(ref_obj, extra_overwrite) {
    ...
    // # origin
    let _desc_origin = get_obj_prop_desc(ref_obj, "origin");
    let _desc_get_origin = _desc_origin.get;
    let _desc_set_origin = _desc_origin.set;
    set_obj_prop_desc(obj, "origin", {
        enumerable: true,
        configurable: true,
        get: function() {
            if (ref_obj.origin === _location.origin) {
                return location_init.origin;
            } else {
                if ("origin" in func_reg && typeof func_reg["origin"] === "function"){
                    return func_reg["origin"];
                } else {
                    return _desc_get_origin.apply(ref_obj);
                }
            }
        },
        set: function(value){
            if (typeof value === "function"){
                func_reg["origin"] = value;
                value = this.__execute__.bind(this, value);
            }
            return _desc_set_origin.apply(ref_obj, [value]);
        }
    });
